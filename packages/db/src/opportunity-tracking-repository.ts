...
export class OpportunityTrackingRepository {
  ...
  async listOutcomes(opportunityId: string): Promise<OpportunityOutcome[]> {
    const result = await this.pool.query(
      "SELECT outcome_id,opportunity_id,observed_at,status,evidence_coverage,expected_quantity,observed_quantity,evidence,observed_subresult FROM opportunity_outcomes " +
